"""Contract test for GET /groups/{group_id}/match-records per
contracts/member-view-api.md (005-member-view US3)."""
...
async def test_match_records_with_guest_token_succeeds(
    ...
    response = await client.get(
        f"/groups/{created['group_id']}/match-records",
        params={"guest_session_token": joined["guest_session_token"], "page": 1},
    )
...
async def test_match_records_without_token_returns_membership_required(
    client: AsyncClient, valid_turnstile_token: str
) -> None:
    created, _joined = await _create_group_and_join(client, valid_turnstile_token)

    response = await client.get(f"/groups/{created['group_id']}/match-records")
    assert response.status_code == 403
    assert response.json()["error_code"] == "MEMBERSHIP_REQUIRED"


async def test_match_records_rejects_non_positive_page(
    client: AsyncClient, valid_turnstile_token: str
) -> None:
    created, joined = await _create_group_and_join(client, valid_turnstile_token)
    ...
    response = await client.get(
        f"/groups/{created['group_id']}/match-records",
        params={"guest_session_token": joined["guest_session_token"], "page": 0},
    )
    assert response.status_code == 422


async def test_match_records_with_logged_in_member_succeeds(
    client: AsyncClient, db_session: AsyncSession, valid_turnstile_token: str
) -> None:
    created, _joined = await _create_group_and_join(client, valid_turnstile_token)
    access_token = await _join_as_logged_in_member(
        client, db_session, created["group_id"], "matchrecords-member-auth@example.com",
