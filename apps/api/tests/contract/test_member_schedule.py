"""Contract test for GET /groups/{group_id}/member-schedule per
contracts/member-view-api.md (005-member-view US1)."""
...
async def test_member_schedule_with_guest_token_succeeds(
    ...
    assert "courts" in body
    assert "roster" in body


async def test_member_schedule_without_token_returns_membership_required(
    client: AsyncClient, valid_turnstile_token: str
) -> None:
    created, _joined = await _create_group_and_join(client, valid_turnstile_token)

    response = await client.get(f"/groups/{created['group_id']}/member-schedule")
    assert response.status_code == 403
    assert response.json()["error_code"] == "MEMBERSHIP_REQUIRED"


async def test_member_schedule_with_wrong_token_returns_membership_required(
    client: AsyncClient, valid_turnstile_token: str
) -> None:
    created, _joined = await _create_group_and_join(client, valid_turnstile_token)
    ...
    response = await client.get(
        f"/groups/{created['group_id']}/member-schedule",
        params={"guest_session_token": "not-a-real-token"},
    )
    assert response.status_code == 403
    assert response.json()["error_code"] == "MEMBERSHIP_REQUIRED"


async def test_member_schedule_with_logged_in_member_succeeds(
    client: AsyncClient, db_session: AsyncSession, valid_turnstile_token: str
) -> None:
    created, _joined = await _create_group_and_join(client, valid_turnstile_token)
    access_token = await _join_as_logged_in_member(
        client, db_session, created["group_id"], "member-schedule-auth@example.com",
    ...
    assert response.status_code == 200
    assert response.json()["current_round_number"] == 1
