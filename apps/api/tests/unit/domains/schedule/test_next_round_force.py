"""Unit test: Next Round force-abandons whatever is still queued/in_progress
before immediately generating the next round (spec FR-032/033)."""
...
from app.domains.schedule.service import create_match_with_participants, generate_next_round


async def _make_group(session: AsyncSession, **overrides: object) -> Group:
    defaults: dict[str, object] = {
        "name": "Next Round Force Test",
        "max_members": 4,
        "match_mode": "singles",
...
@pytest.mark.asyncio
async def test_next_round_abandons_unfinished_matches(db_session: AsyncSession) -> None:
    ...
    unfinished = await create_match_with_participants(
    ...
    result = await db_session.execute(select(Match).where(Match.id == unfinished.id))
...
@pytest.mark.asyncio
async def test_next_round_manual_mode_creates_no_matches(db_session: AsyncSession) -> None:
