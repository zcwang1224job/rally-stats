...
| IX. 可攜性與可部署性 | 一支可逆的 `add_column` migration（可空、無預設值、不鎖表重寫）；無新套件、無新環境變數。部署順序：migration 先於新版後端——新版後端寫入該欄位，舊版前端不送該欄位亦相容。**這個順序是硬性的**（analyze G1）：已查證 API 容器啟動時不會自動跑 migration（Dockerfile 的 `CMD` 只有 gunicorn），而新欄位一旦進了 ORM model，所有讀取 `ShotPlacementRecord` 的路徑都會 SELECT 它——後端若先於 migration 上線，壞掉的是**所有比賽詳情、整個儀表板、以及詳細計分模式下的 `-1`**，不只是新功能。回滾時順序相反：先退後端，再 `alembic downgrade -1`。 | PASS |
...
**`/speckit-analyze` 後修正**：0 項 CRITICAL。補上五項——前後端共用的界內／界外邊界測試向量（I1：補記請求被拒時整筆細節會無聲消失，是回頭讀呼叫端才發現的既有行為）；綁住兩份 `EndingType` 定義的測試（I2）；「最近 10 場／全部」提升為儀表板層級、落點與失誤組成共用（I3）；部署順序的影響範圍（G1）；SC-002 的人工計時獨立為 T037（G2）。
