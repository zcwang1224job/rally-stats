...
**1. 計分時記下這一分怎麼結束的**（US1）— a fourth optional detail on the existing shot-placement picker: one row of five chips under the court (主動得分／對手出界／對手掛網／發球失誤／其他失誤). Where the landing leaves no doubt it is pre-selected — out of bounds → 對手出界, a serve-fault landing → 發球失誤 — so those points cost **zero** extra taps; an in-bounds landing on the loser's half is deliberately *not* read as a winner (it looks the same as a net shot dropping on the hitter's own side), so that one stays a single tap. A hand-pick survives re-tapping the landing unless the new landing contradicts it. Skipping is always allowed; `-1` withdraws it with the rest of the row; simple-scoring matches are untouched. When the landing genuinely contradicts who was credited the point (e.g. A serves, A scores, landing lands on A's own half — not even a valid "serve fault" reading, since a fault always favors the receiver, never the server), every chip is disabled and the warning now shows on the landing tab itself, not only the players tab.

**Faster "+", and no double taps.** The picker now opens in the same tap as "+", before the score request returns; a confirm or cancel made before the point's `score_event_id` is known waits for it (`PendingPoint`), and the picker closes if the point turns out not to be applied. Every score button (plain ±1, detailed "+", the picker's cancel) goes through one `ScoreTapGuard` per court: one request at a time, plus a 400 ms cooldown after each. On the server, `publish()` inside a request now only queues the Ably event, and `PublishAfterResponseMiddleware` sends the request's events in order right after the response — measured in the docker backend, a "+" spent ~66 of its ~76 ms waiting on Ably (and 130–220 ms on each worker's first publish).
...
- **Serve faults follow who served.** A fault always hands the point to the receiver, so 發球失誤 is disabled in the picker — and refused by the server (`ENDING_TYPE_CONTRADICTS_SERVE`) — whenever the credited side served that rally. A serve must also land in the **diagonal** service court (server's score even → the receiver's right court, odd → its left; A's right court is the bottom half of the diagram, B's the top): a landing in the receiver's other court now counts as a serve-fault landing too, alongside the short and doubles-long bands. The picker gets the server and its pre-point score from its caller; the server reads them from the `ScoreServeRecord` at the pre-point score (the same lookup `-1` uses). A match's first point has no known server, so neither check applies there. A serve-fault landing sits on the credited side's *own* half — it never returned the shuttle — so only 發球失誤 or 其他失誤 can explain it: 主動得分, 對手出界 and 對手掛網 are disabled there and refused by the server (`ENDING_TYPE_CONTRADICTS_LANDING`).
...
- Frontend: `ng lint`, `tsc --noEmit` (app + spec) and a production build clean; full `ng test` — **50 files / 565 tests pass**. The bundle-budget warnings and one pre-existing unhandled `NG04002` error (`admin-page.component.spec.ts`) predate this branch.
