...
**實作期間更正（T005／T006）**：analyze F1 建議的「逐場加權」基準被 T005 的零偏差測試證明比原本的合併相除**更差**（發球 −1.5 個百分點 vs. +1.1）——F1 只考慮了比賽之間的組成效應，漏了單場之內「發球分＝贏了上一分之後的那一分」的效應。以模擬實測三種候選後定案：發球類用條件期望 `(得分−1)÷(總分−1)`（偏差 ±0.06），局末用合併全場得分率（±0.5），平手時得分率與領先／落後同列排除（任何基準皆 ±0.9–1.7）。FR-011、FR-012、data-model 規則表、research Decision 1 已同步。
