...
- [X] T037 [US3] Unit test（**先紅燈**）擴充 `apps/api/tests/unit/domains/member/test_insights.py`：`benchmark_quartile`——`pool_size` 3 不產生、4 產生；`q = pool_size // 4`（無條件捨去）：5 人時只有第 1 名與倒數第 1 名、8 人與 9 人時前 2 名與倒數 2 名；最後一名並列兩人時兩人皆為 weakness；全員數值相同 → 不產生；「明顯」需第 1 名或倒數第 1 名**且** `pool_size ≥ 8`，4 人的團一律為輕微；排序的樣本數取 `mine.matches_used`；只取 `status="ok"` 的指標；**同一 `metric_key` 同時有 benchmark 與 self 候選時只留 benchmark**；排序中 benchmark 先於 self；`recent` 清單不受去重影響；`benchmark_group_name` 被帶出；`params.mine`／`group_average`／`rank`／`pool_size` 與 `BenchmarkMetric` 逐位相等；`when_trailing` **可以**經由此來源出現（FR-012 只排除自我對比）
