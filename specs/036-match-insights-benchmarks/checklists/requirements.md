...
- 沒有使用 [NEEDS CLARIFICATION]。以下四項是以合理預設寫入 Assumptions 的產品決定，建議在 `/speckit-clarify` 或規劃前由產品負責人確認：
  1. 團內比較不具名（只顯示平均與本人名次，不做各項技術指標的具名排行榜）。
  2. 團內比較的對象包含已離團球員與訪客名單球員（與 018 只列現役成員不同）。
  3. 好友戰績頁會顯示該好友的「待加強」摘要（沿用 023「明細與彙總統計同一項設定控管」的定案，不新增隱私開關）。
  4. 各項門檻數值（5／10 個百分點、30 分、3／5 場、前後四分之一等）為首版預設。
