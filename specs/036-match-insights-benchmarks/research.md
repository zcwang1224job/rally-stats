...
- **自我對比的基準怎麼算**（實作時以模擬實測後定案，取代 analyze F1 的「逐場加權」）：基準回答的是「這個情境對我毫無影響時，這個數字應有的值」。以「單場內每一分勝率固定」的合成球員（弱 0.30–0.50、中 0.35–0.65、強 0.50–0.70，各四萬場）實測各候選基準與指標的差距：

  | 指標 | 合併相除 | 逐場加權 | 條件期望 |
  |---|---|---|---|
  | 發球時 | +0.5 到 +1.3 | −1.1 到 −1.7 | ±0.05 |
  | 接發球時 | −0.5 到 −1.3 | +1.2 到 +1.7 | ±0.06 |
  | 局末 | −0.4 到 +0.1 | −0.5 到 +0.2 | 不適用 |
  | 平手時 | −0.9 到 +0.8 | −1.7 到 +1.7 | 不適用 |

  （單位：百分點。）發球類有兩個方向相反的偏差：得分方發下一球，所以打得順的比賽發球分較多（合併相除偏高）；但發球分就是「我方贏了上一分之後的那一分」，總得分固定時一個勝分已被用掉，期望得分率是 `(得分 − 1) ÷ (總分 − 1)` 而不是 `得分 ÷ 總分`（逐場加權反而偏低，而且比合併相除更差）。用這個條件期望逐場計算、以該場的指標分母加權，偏差即為零；033 FR-012 本來就把每場第一分排除在發球／接發球分母之外，使這個式子精確成立。局末不論用哪個基準都在 0.5 個百分點以內，採最簡單的合併全場得分率。
- **平手時得分率因此被排除**：一場比賽有多少分處於平手，取決於決定該場比分的同一批運氣，試過的每一種基準（含「該場其餘分數的得分率」）都留下 0.9 到 1.7 個百分點、方向固定的偏差——與領先／落後同一類，一併列入 FR-012。教訓：analyze F1 只想到比賽之間的組成效應，漏了單場之內的效應；憑推論選的基準比原本的更差，是 T005 的零偏差測試擋下來的。
