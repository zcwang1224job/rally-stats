...
- 範圍：與 014「我的團」相同——我曾有 `member_id` 為我的名單列的團（不論狀態），以及我建立的團（不論是否已解散）。以訪客身分參加的團不在其中。
...
**回應中除了 `group` 與檢視者本人的數值之外，不含任何可識別或可還原其他球員的欄位。** 沒有名單、沒有個別數值、沒有最高／最低值。每位球員的個別數值只存在於伺服器端的運算過程中。契約測試以「回應 JSON 全文不含任何其他參賽者的暱稱、`member_id`、`roster_entry_id`」驗證。
