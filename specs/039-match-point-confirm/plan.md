...
5. **US2（P2）**：在**同一個試點畫面**補齊「不該跳的時候不跳」的負向測試（deuce、上限、非決勝分、「-1」、詳細模式），以及防重複、離線、伺服器判定優先三條容易漏掉的 MUST。
6. **US3（P3）**：把已經驗證過的那一套複製到其餘三個畫面。

> 註：US2（只在該出現時出現）與 US1 在實作上是同一段分支的一體兩面，無法真正分開實作；上面的順序反映的是**驗證**重心的推進，不是三次獨立開發。
>
> 註 2：US2 **必須排在 US3 之前**。先在一個畫面把正向與負向行為都鎖住，再複製三份；反過來先複製、後補測試，等於把同一個錯誤複製四次才發現。tasks.md 的階段順序即依此排定。
