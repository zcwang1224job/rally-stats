...
- **按下去就會立刻結束這一輪的情況——先提醒**（`/speckit-analyze` D1；使用者 2026-09-19 決定：維持自動換輪，但要先提醒；FR-031～FR-033）：所有場地閒置、只剩含 A 的場次、開著自動換輪——A 一按休息，這一輪立刻結束，那幾場被取消且救不回來。這是不可還原的後果，同樣會取消場次的「離開團」就有確認框（`leave-group.component.html` 的 `app-confirm-dialog`），Constitution V 要求比照。
  ...
  - ~~**不提醒的情況**：還有比賽在打時按休息、之後最後一場打完而換輪。~~ **2026-09-20 修訂**：實作後確認「立刻結束」在正常流程中幾乎只出現在「已規劃、未開始」的輪次——只要場地空著又有他能打的場次，系統早就叫上場了；最常見的是「別人還在打時按休息，最後一場打完才因他換輪」。使用者決定這種情況**按下時也要提醒**。規則（`rest.py` 的 `_refuse_if_round_would_end()`）：先照原本的「立刻」判斷（`immediate: true`）；否則，開著自動換輪、排程方式在輪到時保留而非替補（`_substitutes_for_rest()` 為假）、其他準備中的人排得出下一輪（`_can_generate_any_match()`）、而他在這一輪還有排隊中的場次，就以 `immediate: false` 拒絕，`matches_to_cancel` 為他自己的場次數。這是預估——他在這一輪結束前回來就不會被取消——所以文字寫「沒回來的話」。替補模式不提醒：輪到時由替補上場，他的場次不會被取消。休息後的說明文字（FR-033，由前端從自己的 `rest_effect == "held"` 場次數得出）保留，作為之後的持續提醒。
