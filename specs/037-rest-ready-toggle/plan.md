...
2. **候選人名單（US1、US3）**：休息對排程的全部影響就是「候選人名單少了他」。條件收斂在讀取名單的三個函式與 `wait_count` 累加的一處，各排程方式的配對邏輯一行不改。
...
5. **被休息卡住的輪次（FR-020）**：`round_is_complete()` 要求所有場次都結束，被保留的場次會讓這一輪永遠不結束、自動換輪永遠不觸發。新增「被休息卡住」的判斷並接進既有的自動換輪，另加兩個守門條件避免輪次編號空轉（research Decision 6）。**FR-020 是規格撰寫時的自行決定；使用者於 2026-09-19 直接進入規劃階段，視為接受，列於此供核對。**
...
- 後端：pytest。
  ...
  - `tests/unit/domains/schedule/test_rest_round_stall.py`（新）——`round_is_stalled_by_rest()` 的四個條件各缺一時為假；開啟自動換輪時比賽結束與狀態變更兩個觸發點皆會換輪；守門條件：下一輪排不出任何一場時不換輪、反覆切換不使輪次編號增加；未開啟時不換輪。
  - `tests/unit/domains/schedule/test_fairness_simulation.py`（擴充）——固定亂數種子、多人多輪、隨機休息與回來：休息前後 `wait_count` 差值恆為 0；回來後第一次挑人的順位不優於同一時刻 `wait_count` 相同且一直準備中的人；回來後的上場比例不高於全體中位數加容許值（防 Decision 4 回歸）。
...
| V. 破壞性操作二次確認 | 切換**不**加確認框——它隨時可還原，且是設計上要「隨手按」的操作。唯一接近破壞性的後果（開著自動換輪、所有場地閒置時按休息會使這一輪立刻結束、他被保留的場次被取消）是管理員自行開啟的自動行為，與「最後一場打完即自動換輪」同類；已於 research Decision 6 記載並接受。管理員手動結束一輪的既有確認框不變。 | PASS |
...
| VIII. i18n 與時區 | 後端只回傳狀態、代碼與時間；所有新文字在 `zh-TW.json`／`en.json` 各一份。不新增錯誤代碼（沿用 `ROSTER_ENTRY_NOT_FOUND`、`GROUP_DISBANDED`）。`resting_since` 與休息區間以 UTC 儲存、只用於比較先後，首版不顯示給使用者，不涉及顯示時區。 | PASS |
...
│   │                        #       _get_active_roster_ids()、_get_active_roster_ordered()、
...
US1 完成即有最小可用價值（下一輪不排他）；US1＋US2 是規格定義的完整最小版本；US3 MUST 與 US1 同一次上線——少了它，休息就是插隊的捷徑。
