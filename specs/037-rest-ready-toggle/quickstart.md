...
- **之後可能被取消也要提醒**（2026-09-20）：同樣的團，但還有一場別人的比賽在打時 A 按休息 → 跳出「要休息嗎？這一輪結束前你沒回來的話，你還沒打的 2 場比賽會被取消」；確認後 A 休息中、這一輪**沒有**結束，A 的畫面出現說明文字「你還有 2 場保留中，這一輪結束前沒回來會被取消」。
- **不該跳的情況**：關閉自動進入下一輪後重做 → 不跳提醒。個人全混搭或公平輪替雙打（輪到時由替補上場）→ 不跳提醒。
