...
- [ ] T038 實機驗證 quickstart.md 情境 1–12 與 7b（依記憶中的 worktree 實機流程：後端 uvicorn :8001、前端 `ng serve --port 4300`、Playwright 三個 context 分別為管理員、會員 A、訪客 B；**先做實機、再跑會清空資料庫的 pytest**）。情境 1 量測切換到其他視窗更新的時間（SC-001：< 2 秒）；情境 12 以 375px 寬截圖確認名單列沒有水平溢出。把截圖與發現的問題記在 PR 描述
