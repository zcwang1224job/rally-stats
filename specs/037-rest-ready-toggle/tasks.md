...
- [ ] T032 [P] [US4] 於 `apps/web/src/app/features/group-admin/schedule-management/schedule.service.ts` 緊接 `kickMember()`（約 L236）新增 `setMemberRestState(groupId, rosterEntryId, resting, confirmRoundEnd = false): Observable<RestStateResponse>`；補 service spec
