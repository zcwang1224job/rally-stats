...
- **上線單位是 US1＋US2＋US3 一起**（`/speckit-analyze` E2）——少了 US3，休息就是插隊的捷徑；少了 US2，畫面標示「休息中」的人仍會被叫上場打已排好的場次（違反 SC-002）。三者各自可獨立開發、示範與驗收，但不分開上線。
- 階段順序仍是 US1 → US3 → US2：US3 改動小、先把公平性鎖住；US2 改到所有叫場的唯一入口與自動換輪，風險最高，放在最後、測試最密。US2 與 US3 的後端互不相依，可由兩人平行。
...
- [X] T014 [US1] 新增 `apps/web/src/app/core/rest-toggle-button/rest-toggle-button.component.{ts,html,scss,spec.ts}`（`app-rest-toggle-button`；inputs：`resting: boolean`、`currentlyPlaying: boolean = false`、`pending: boolean = false`、`compact: boolean = false`（管理頁名單列用）；output：`toggled: boolean`——發出**目標狀態**）。原生 `<button type="button">`、`[attr.aria-pressed]="resting"`、`pending` 時 `disabled`；文字為 `restToggle.rest`（「我要休息」）／`restToggle.ready`（「準備好了」），圖示＋文字；`resting && currentlyPlaying` 時在按鈕下方顯示 `restToggle.afterThisMatch`（「打完這一場後開始休息」，FR-014）。不含確認框（FR-002）。同時於 `apps/web/src/assets/i18n/zh-TW.json` 與 `en.json` 新增 `restToggle.*` 與 `scheduleManagement.restingBadge`（「休息中」）。spec：兩種狀態的文字與 `aria-pressed`、`pending` 停用、`afterThisMatch` 的出現條件、點擊發出目標狀態；另比照 `player-insights.component.spec.ts` 讀入兩份語系檔，斷言本功能**目前已新增**的 key 在兩邊都存在（後續任務新增 key 時同步加進這條測試的清單）
- [X] T015 [US1] 於 `apps/web/src/app/features/group-member-view/member-schedule/member-schedule.component.{ts,html}`：名單區（html 約 L68–78）每一列在既有的 `currently_playing`／`wait_count` 標記旁，`resting` 時顯示「休息中」標記（圖示＋文字，FR-008）；**只有自己的那一列**（以 `resolveRosterEntryId()` 的結果比對）顯示 `<app-rest-toggle-button>`，另在名單上方放一個同樣的按鈕讓球員不必找自己的列；`toggled` → `setOwnRestState()`，成功後以回應更新自己的列並 `load()`，失敗時還原並顯示 `error.i18nKey | translate`；`pending` 期間停用。把 `roster.restChanged` 加入 group 頻道的訂閱事件清單（ts 約 L19–20），處理方式同既有事件＝`load()`。擴充 `member-schedule.component.spec.ts`：標記的出現、按鈕只在自己的列、成功／失敗流程、收到 `roster.restChanged` 會重抓（depends on T013、T014）

- [X] T040 [US1] 管理頁的休息標示與即時更新（`/speckit-analyze` E1——US1 情境 1 要求管理頁名單即時顯示；原本分散在 T030 與 T033，會讓第一階段的管理頁看不到誰在休息）：於 `apps/web/src/app/features/group-admin/admin-page/admin-page.component.{ts,html}` 的名單分頁（html 約 L255–300），每一列在既有標記（約 L261–267）旁，`resting` 時顯示「休息中」標記（圖示＋文字，與 T015 同一個語系 key `scheduleManagement.restingBadge`）；把 `roster.restChanged` 加入管理頁 group 頻道的訂閱（ts 約 L312–324）→ 重抓名單、賽程、本輪賽程清單。**不含**切換按鈕（那是 US4 的 T033）。擴充 `admin-page.component.spec.ts`：標記的出現、收到事件會重抓。*編號 T040 為 analyze 後追加，刻意不重編既有任務以免打亂交叉引用；執行順序以所在階段為準。*（depends on T013）

**Checkpoint**：休息者不進新的一輪，成員頁與管理頁的所有人即時看到（quickstart 情境 1–3、10）。**尚不可上線**——回來的人目前會因為 `rest` 與 `played` 而得到優先權（Phase 4），且這一輪已排好的場次仍會叫到休息中的人（Phase 5）。
...
- [X] T023 [US2] 於 `apps/api/app/domains/schedule/service.py`：新增 `round_is_stalled_by_rest(session, group) -> bool` 與 `_can_generate_any_match(session, group) -> bool`（data-model.md「這一輪是否被休息卡住」；research Decision 6）；`check_round_complete_and_maybe_auto_advance()` 改為 `(round_is_complete or round_is_stalled_by_rest) and _can_generate_any_match`——**注意**：由比賽結束觸發、且 `round_is_complete` 為真的既有路徑，`_can_generate_any_match` 為假時的行為要與現在一致；先以 `test_round_completion.py` 確認現況（現況若會產生空的輪次，就只對「被卡住」與「由狀態變更觸發」兩種情況套用守門，並在 docstring 寫明）。把「現在會不會自動換輪」抽成公開的 `round_would_auto_advance(session, group, *, triggered_by_rest_change: bool) -> bool`（不含任何寫入），`check_round_complete_and_maybe_auto_advance()` 改為呼叫它——**提醒與換輪 MUST 共用這一個函式**（contracts/rest-state-api.md）。於 `apps/api/app/domains/schedule/rest.py`：(1) 切成休息時，寫入 `resting_since` 並 `flush()` 之後、commit 之前，呼叫 `round_would_auto_advance(…, triggered_by_rest_change=True)`；為真且 `confirm_round_end` 為假 → 計算 `matches_to_cancel`（當前這一輪 `queued` 的場次數）、`await session.rollback()`、`raise ApiError("REST_ENDS_ROUND", status_code=409, detail={"matches_to_cancel": n})`；(2) 預留位置接上步驟 5（commit 之後呼叫 `check_round_complete_and_maybe_auto_advance()`，帶 `triggered_by_rest_change=True`）。注意授權檢查在 `set_rest_state()` 被呼叫**之前**就已完成（T011、T031），因此無法證明擁有該列的呼叫者不會走到這裡。使 T021 全綠（depends on T020、T021）
...
- [X] T029 [US2] 成員頁：於 `apps/web/src/app/features/group-member-view/member-schedule/member-schedule.component.{ts,html}`——場地的等待原因（html 約 L12–33）以 exhaustive `Record<WaitingReason, string>` 對應語系 key，**不認得的值退回 `waitingNoQueuedMatch`**（舊前端搭新後端／未來新值）；`next_up.substitutions` 非空時在預告下方顯示 `restToggle.substitutionNote`（「{{substitute}} 代替 {{resting}}（休息中）」），暱稱經 `<app-nickname>` 或插值，不使用 `innerHTML`；固定搭檔時，自己的 `partner_roster_entry_id` 指到的那一列為 `resting` → 在自己的切換按鈕附近顯示 `restToggle.partnerResting`（「搭檔休息中，你們的場次暫緩」，FR-022）；成員頁的本輪賽程清單對 `rest_effect` 顯示 `restToggle.effectHeld`（「保留，等他回來」）／`restToggle.effectSubstitute`（「輪到時由替補上場」）。新增語系 key 到兩份語系檔並加進 T014 的語系測試清單。擴充 `member-schedule.component.spec.ts`：兩個新等待原因、未知值的退回、替補註記、搭檔說明的出現與不出現、`rest_effect` 標示（depends on T015、T028）
- [X] T030 [US2] 管理頁：於 `apps/web/src/app/features/group-admin/schedule-management/round-matches-list.component.{ts,html}` 顯示 `rest_effect` 標示（與 T029 同一組語系 key）；於 `apps/web/src/app/features/group-admin/admin-page/admin-page.component.{ts,html}` 的賽程區塊，`waiting_on_rest` 非 `null` 時顯示提示 `scheduleManagement.waitingOnRest`（「剩下 {{count}} 場在等休息中的球員：{{names}}」），`stalled` 為真時改用醒目樣式（圖示＋文字，不只顏色）並在**未開啟自動進入下一輪**時附上 `scheduleManagement.waitingOnRestHint`（「可以幫他按『準備好了』，或結束這一輪」，FR-021）；場地區塊的等待原因與 T029 共用同一個對應（抽到 `apps/web/src/app/core/` 下的小工具 `waiting-reason-label.ts`，成員頁同步改用）。（`roster.restChanged` 的訂閱已由 T040 加入。）`waiting_on_rest` 的提示依 FR-021 放寬後的定義：`match_count > 0` 就顯示，`stalled` 時醒目。擴充 `round-matches-list.component.spec.ts` 與 `admin-page.component.spec.ts`（depends on T028、T029、T040）
- [X] T041 [US2] 成員頁的「會結束這一輪」提醒（FR-031～FR-033；使用者於 `/speckit-analyze` 後的決定）：新增 `apps/web/src/app/core/rest-toggle-button/rest-ends-round.ts`——`restEndsRoundCount(error: ApiError): number | null`（`errorCode === 'REST_ENDS_ROUND'` 時回傳 `detail.matches_to_cancel`，型別不符或缺值回 `null`），成員頁與管理頁共用。`group-member-view.service.ts` 的 `setOwnRestState()` 新增選填參數 `confirmRoundEnd = false` 並帶進 body。於 `member-schedule.component.{ts,html}`：`setOwnRestState(true)` 失敗且 `restEndsRoundCount()` 不為 `null` → **不**顯示錯誤訊息，改為開啟既有的 `<app-confirm-dialog>`（用法比照 `leave-group.component.html`），標題 `restToggle.endsRound.title`（「要休息並結束這一輪嗎？」）、內文 `restToggle.endsRound.body`（「本輪還沒打的 {{count}} 場比賽會被取消，並直接進入下一輪。」）、確認鈕 `restToggle.endsRound.confirm`（「確認休息」）、取消鈕沿用既有；確認 → `setOwnRestState(true, true)`；取消 → 按鈕回到「我要休息」、`pending` 解除、不重抓也不顯示錯誤。第二次請求若成功但沒有換輪（FR-033）不需特別處理——照一般成功流程 `load()`。另加 FR-033 的不打斷說明：自己休息中、且本輪賽程清單裡自己的場次有 `rest_effect === 'held'` 的 N 場 → 在切換按鈕下方顯示 `restToggle.heldNote`（「你還有 {{count}} 場保留中，這一輪結束前沒回來會被取消」）。新增語系 key 到兩份語系檔（含後備用的 `errors.REST_ENDS_ROUND`）並加進 T014 的語系測試清單。擴充 `member-schedule.component.spec.ts`：409 時開啟提醒而非錯誤訊息、內文帶正確的場次數、確認後以 `confirmRoundEnd=true` 重送、取消後狀態完全還原、其他錯誤仍走原本的錯誤顯示、`heldNote` 的出現條件；新增 `rest-ends-round.spec.ts`（depends on T015、T029）

**Checkpoint**：US1＋US2＋US3——規格定義的完整最小版本，**可以上線**（quickstart 情境 1–8、7b、10）。
...
- [X] T032 [P] [US4] 於 `apps/web/src/app/features/group-admin/schedule-management/schedule.service.ts` 緊接 `kickMember()`（約 L236）新增 `setMemberRestState(groupId, rosterEntryId, resting, confirmRoundEnd = false): Observable<RestStateResponse>`；補 service spec
- [X] T033 [US4] 於 `apps/web/src/app/features/group-admin/admin-page/admin-page.component.{ts,html}` 的名單分頁（html 約 L255–300；「休息中」標記已由 T040 加入）：在踢人按鈕（約 L296–300）之前放 `<app-rest-toggle-button [compact]="true" …>`，**包含建立者的列**（踢人按鈕在該列隱藏，切換按鈕不隱藏）；`toggled` → `setMemberRestState()`，逐列的 `pending` 狀態（`Set<roster_entry_id>` signal）；失敗時顯示錯誤並還原；一般情況不加確認框。**`REST_ENDS_ROUND`**（FR-032）：以 T041 的 `restEndsRoundCount()` 判斷，是的話開啟 `<app-confirm-dialog>`，內文用管理員版 `restToggle.endsRound.adminBody`（「將 {{nickname}} 設為休息中，會取消本輪還沒打的 {{count}} 場比賽，並直接進入下一輪。」），確認 → `setMemberRestState(…, true, true)`，取消 → 該列還原。compact 模式的按鈕文字用 `restToggle.adminRest`（「休息」）／`restToggle.adminReady`（「準備好了」），`aria-label` 帶上暱稱（`restToggle.adminAriaLabel`：「將 {{nickname}} 設為休息中」）。擴充 `admin-page.component.spec.ts`：每列都有按鈕（含建立者）、成功／失敗、逐列 pending 互不影響、409 時開啟管理員版提醒且內文含暱稱與場次數、確認後帶 `confirmRoundEnd=true` 重送、取消後該列還原（depends on T014、T030、T032、T041）
- [X] T034 [P] [US4] 手動操作的選單：於 `apps/web/src/app/features/group-admin/schedule-management/manual-assign.component.{ts,html}`（`availableRoster()` 約 L30）與 `round-matches-list.component.ts` 的 `changeCandidates()`（約 L198）及交換選單——休息中的候選人在選項文字後加上「（休息中）」（`scheduleManagement.restingSuffix`），**仍可選**（FR-029）；排序不變。新增 `manual-assign.component.spec.ts`（該元件目前沒有測試檔）：標示出現、休息中的人可被選取並送出；擴充 `round-matches-list.component.spec.ts` 同樣兩點。後端不需改動——於 `apps/api/tests/unit/domains/schedule/test_manual_assign_validation.py` 加一條：把休息中的人手動排上場 → 成功，且他的 `resting_since` **不變**（FR-029）
...
- [X] T035 整合測試（Constitution II 的端到端要求；SC-008）：新增 `apps/api/tests/integration/test_rest_toggle_flow.py`——經 HTTP 走完：開團（個人全混搭、2 面場地）→ 6 人加入 → 規劃並開始一輪 → A 以自己的身分設休息 → 逐場計分到只剩含 A 的場次 → 確認上場的是替補後的陣容且與前一次 `member-schedule` 的 `next_up` 相同 → A 設準備中 → 把這一輪打完 → 讀排行榜、團內對戰紀錄、A 與替補者各自的個人對戰紀錄：被替補的那一場記在替補者名下、A 沒有那一場、其餘數字與以同一批比分手算的結果一致。另一條：單打循環＋自動換輪，A 在還有別的比賽進行時休息（不回 409）→ 最後一場別人的比賽打完 → 這一輪被卡住而自動換輪 → A 回來被補進新的一輪。第三條（FR-031～FR-033）：同樣的團打到所有場地閒置、只剩含 A 的場次 → A 設休息得到 409 `REST_ENDS_ROUND` 與正確的 `matches_to_cancel`，此時另一位成員讀到的名單裡 A 仍是準備中 → A 帶 `confirm_round_end: true` 重送 → 200、輪次編號加一、新的一輪沒有 A
- [X] T036 [P] 既有測試的回歸確認：於 `apps/api` 執行 `python -m pytest tests/unit/domains/schedule tests/unit/domains/group tests/contract tests/integration -q`，**任何既有測試的斷言都不應需要修改**；若有需要修改的，逐一列在本任務的備註並說明為什麼不是回歸。再跑一次全套件（約 20 分鐘，背景執行並把 `-rf` 輸出導到檔案；跑完之前不要結束回合）
- [X] T037 [P] 靜態檢查：`apps/api`：`ruff check app/ tests/ && mypy app/`；`apps/web`：`npm run lint && npx tsc --noEmit -p tsconfig.app.json && npm test -- --watch=false`。`_choose_next_queued_match()` 回傳型別改變後，`mypy --strict` MUST 沒有任何 `# type: ignore` 新增
- [X] T038 實機驗證 quickstart.md 情境 1–12 與 7b（依記憶中的 worktree 實機流程：後端 uvicorn :8001、前端 `ng serve --port 4300`、Playwright 三個 context 分別為管理員、會員 A、訪客 B；**先做實機、再跑會清空資料庫的 pytest**）。情境 1 量測切換到其他視窗更新的時間（SC-001：< 2 秒）；情境 12 以 375px 寬截圖確認名單列沒有水平溢出。把截圖與發現的問題記在 PR 描述
- [X] T039 [P] 文件：更新本機的 `docs/features.md`（gitignored，直接寫進主 checkout，不 commit）的排程章節，加入休息／準備切換的行為摘要與四個容易被問到的問題（按休息後已排好的場次怎麼辦、回來會不會插隊、自動換輪何時會因休息而觸發、什麼時候按休息會先跳提醒）；新增 `specs/037-rest-ready-toggle/pr-description.md`（比照 036 的格式），依憲章「技術治理」要求寫明本變更如何維持「伺服器為唯一可信來源」（`roster.restChanged` 只由後端發布、替補與保留的判斷全在後端）與「管理員操作僅限管理頁」（管理員端點 `require_admin`、免驗證畫面無切換）
...
- **Phase 6 US4**：後端 T031 只依賴 T010，可在 US1 完成後任何時間做；前端 T033 依賴 T014、T030（同一個檔案 `admin-page.component.*`，避免衝突所以排在 T030 之後）與 T041（共用 `restEndsRoundCount()`）。US4 若要提前到 US2 之前，T033 的 `REST_ENDS_ROUND` 處理可先不做——後端在 T023 之前不會回這個錯誤。
...
- `apps/web/…/member-schedule.component.*`：T015 → T029 → T041。
- `apps/web/…/admin-page.component.*`：T040 → T030 → T033。
- `apps/web/…/schedule.models.ts`：T013 → T028。
- `apps/web/…/group-member-view.service.ts`：T013 → T041。
- 兩份語系檔：T014、T029、T030、T033、T034、T040、T041 都會加 key——各自只加自己的區塊，合併衝突以「兩邊都保留」解決。

### User Story 獨立性

| Story | 可獨立示範 | 可獨立上線 | 說明 |
|---|---|---|---|
| US1 | ✅ | ❌ | 少了 US3 回來的人會插隊；少了 US2 標示「休息中」的人仍會被叫上場。 |
| US1＋US3 | ✅ | ❌ | 公平性已鎖住，但這一輪已排好的場次仍會叫到休息中的人——畫面說他在休息、系統卻叫他上場，違反 SC-002（`/speckit-analyze` E2）。 |
| US1＋US3＋US2 | ✅ | ✅ | **上線單位**。 |
| US4 | ✅（需 US1） | ✅（隨上線單位或之後） | 另一個入口。 |
...
線 C（US4 後端）：T031
線 D（前端，US1 的 T013／T014 完成後）：T015 ║ T040 → T028 → T029 → T030 ║ T041 → T033
```

## Implementation Strategy

1. **先把數學做對**（Phase 1–2）：migration 與兩個純函式。沒有行為改變，可以先合併。
2. **US1 → US3**：球員能休息、不進新場次、回來不插隊。不碰叫場邏輯，風險最低；完成後可在 `feature/rest-toggle` 上先交給使用者試用與回饋，但**不合併上線**。
3. **US2**：改動 `_choose_next_queued_match()`（所有叫場的唯一入口）與自動換輪，是本功能風險最高的部分；T019 的併發測試、T021 的空轉測試與 `REST_ENDS_ROUND` 的 rollback 測試是這一段的安全網，MUST 先紅後綠。**US1＋US3＋US2 完成＝第一次上線**。
4. **US4 可同時或稍後**：後端一支路由、前端一個按鈕。若現場「沒帶手機的人」很多，與上線單位一起出。
5. **每個階段結束都跑一次 T036 的既有測試**——「沒有人休息時行為逐位相同」是整個功能最重要的不變量。

## Notes

- 任務總數 41。Setup 2、Foundational 3、US1 11、US3 3、US2 13、US4 4、Polish 5。T040（US1）、T041（US2）為 `/speckit-analyze` 後追加，編號不依執行順序。
- SC-004 的模擬測試擴充的是 `tests/integration/test_schedule_fairness_simulation.py`（既有的情境驅動模擬在這裡，有 `Scenario`／`_run()`／`Report`）；plan.md、research.md、quickstart.md 已同步。
- **實作時與任務描述的差異**：
  - T006 (i)「沒有人休息時，產生的場次與移除本功能前相同」沒有另寫測試——修改前的程式無法與修改後在同一次執行中比對；改以既有排程測試不改斷言全綠為準（T008 後 369 項全綠）。
  - T007 沒有先確認紅燈：`rest.py` 在背景測試佔用測試資料庫期間先寫好了。
  - T015 只在名單上方放一個切換按鈕，沒有在自己那一列再放一個——兩個按鈕做同一件事是重複；名單上每一列只顯示「☕ 休息中」標籤。
  - T015 找到一個由本功能引入的錯誤並已修正：`load()` 在 effect 內同步執行、讀到 `selfRosterEntryId`，寫入它會讓 effect 重跑、多載入一次；改為 `untracked(() => this.load())`。
  - T013：`group-member-view.service.ts` 原本沒有測試檔，新增 `group-member-view.service.spec.ts` 釘住「會員不帶殘留的訪客 token」。
  - T017：從未上場過的人休息回來不給 `played_credit`——他是新人，既有規則本來就讓他優先（FR-026 不適用）；只對打過比賽的人補。
  - T018：沒有在 T017 之前跑一次，改為在測試內以 monkeypatch 關閉 Decision 3／4 的修正（`fixes="off"`），每次執行都同時證明斷言抓得到問題。擴充方式是在同一檔新增專屬的休息模擬（`test_resting_is_not_a_shortcut`），而不是在既有 `Scenario` 加欄位——插入休息／回來會改變既有情境的出賽迴圈。第一版設定（4 小時、3 種子、回來機率 0.35）關閉修正時合併超出值最低只有 +0.028，與開啟時差距不足 0.05，依 B1 的規則改為加長休息（6 小時、6 種子、休息／回來 0.3／0.15）而不是壓縮容許值；三次實測與容許值 0.02 記於測試註解。`rest.py` 也需要以模擬時鐘取代 `datetime`，否則休息區間與比賽時間對不上。
  - T020：`peek_next_queued_match()` 維持回傳 `Match`（既有測試以它的 `.id` 斷言），另加 `peek_next_call()` 回傳含替補的 `NextMatchChoice` 給預告用。一般路徑把「不含休息者」寫進主查詢的條件，查詢次數與修改前相同；只有查不到候選時才另查含休息者的場次。
  - T019 (n)：原本「兩個叫場同時搶同一位替補」的併發測試，拿掉團列鎖也會通過——第一個叫場以 `FOR UPDATE SKIP LOCKED` 鎖住整批含休息者的場次，第二個直接跳過。真正需要團列鎖的是「叫場找替補」與「場地一空就排下一場」（不讀場次列）同時發生，測試改為這個情境。拿掉顯式鎖後它仍通過：開始比賽時寫入 `pair_history`，其外鍵對團列取 KEY SHARE，連續輪轉的 FOR UPDATE 會等它——顯式鎖保留，不依賴這個附帶效果，測試 docstring 已註明。
  - T023：「被休息卡住」原本另加「連續輪轉湊不出四人」的條件，實作時發現多餘——連續輪轉只用於公平輪替雙打，而它會找替補；有四個閒置準備中的人就一定有替補，所以已不算卡住。「下一輪排得出比賽才換輪」的守門只套用在「被休息卡住」與「由休息狀態變更觸發」兩種情況；比賽打完而結束的既有路徑行為不變。提醒只在確實有排隊中的場次會被取消時才跳（0 場不提醒）。
  - T026：三處各自計算的場地等待原因（管理／成員賽程、場地即時狀態、`match.ended` 事件）收斂成一個 `_idle_court_status()`。
  - T029／T030：等待原因的共用工具放在 `core/waiting-reason-label.ts`（不是 `core/` 下另開資料夾），且除了成員頁與管理頁，也套用到單一場地控制面板、全部場地面板、計分板——這三個公開畫面原本也各自寫死兩個原因。`waiting_on_rest` 的提示放在本輪賽程清單元件（`round-matches-list`）裡，而非管理頁本體：資料就在那支回應裡，而會有場次在等休息者的輪次一定處於「已規劃／進行中」，清單此時會自動展開。
  - T041：既有的 `app-confirm-dialog` 確認鈕文字固定為「確認」，新增選填輸入 `confirmLabel`（預設行為不變）以顯示「確認休息」。
  - T035：寫整合測試時發現一個由本功能引入的錯誤——開著自動換輪、還沒有任何一輪的團，按休息／準備好了會自己排出第一輪；改為由休息觸發的換輪需要至少產生過一輪（`test_a_group_nobody_has_started_never_starts_itself`）。也確認了「會結束這一輪」的提醒在正常流程中幾乎只出現在「已規劃未開始」的輪次，第三條整合流程以此構造。
  - T038：以 Playwright 腳本涵蓋 21 個檢查點（見 pr-description.md），沒有逐一人工走完 quickstart 的 12 個情境。截圖與腳本放本機 `docs/037-rest-toggle-check/`。
  - T027：新增欄位的契約測試集中寫在 `test_rest_state_endpoints.py`（以服務層直接造出指定場次，再以 HTTP 讀回應），沒有分散改 `test_member_schedule.py` 等三支既有檔。
- **`/speckit-analyze` 後的調整（2026-09-19）**：D1 → T021(f–f6)、T023、T027、T033、T035、T041（先提醒）；F1 → T006(j)(k)、T008（ready／active 兩個名單函式）；E1 → T040；E2 → Organization、獨立性表、Implementation Strategy（上線單位改為三個 P1 一起）；B1 → T018(c)；E3 → 下方的 SC 追溯。
- FR 追溯：FR-001（T007、T010）、FR-002（T011、T014、T015）、FR-003（T031、T033）、FR-004（T011、T012、T031）、FR-005（T007、T012）、FR-006（T006）、FR-007（T010、T015、T030）、FR-008（T014、T015、T033）、FR-009（T006、T008）、FR-010（T008、T016）、FR-011（T006、T026、T029）、FR-012（T022、T024）、FR-013（T007）、FR-014（T007、T014）、FR-015（T019、T020、T026）、FR-016／FR-017（T019、T020）、FR-018（T019、T020）、FR-019（T022、T010）、FR-020（T021、T023）、FR-021（T026、T030）、FR-022（T026、T029）、FR-023（T016、T019）、FR-024（T008、T016）、FR-025（T003、T004、T017）、FR-026（T016、T022）、FR-027（T019、T020）、FR-028（T016、T027、T035）、FR-029（T034）、FR-030（T007）、FR-031（T021、T023、T027）、FR-032（T041、T033）、FR-033（T021、T023、T041）、FR-034（T006、T008）。另：FR-007／FR-008 的管理頁部分改由 T040 涵蓋。
- SC 追溯：SC-001（T038）、SC-002（T019、T035）、SC-003（T007、T019）、SC-004（T016、T018）、SC-005（T019）、SC-006（T021）、SC-007（T015、T040、T038）、SC-008（T016、T027、T035）、SC-009（T021、T027、T041、T038）。
