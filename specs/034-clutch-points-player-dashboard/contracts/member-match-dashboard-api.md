...
- **授權**：與既有 `GET /members/me/match-records` 相同的 dependency（`require_member`）。
...
| `better_when == null`（`match_points_saved`） | `verdict: null`——前端只顯示數值，不標進步／退步 |
