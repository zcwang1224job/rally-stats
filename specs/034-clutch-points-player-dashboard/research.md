...
- **Rationale**：單場的發球得分率只有約 20 個樣本，逐場畫線幾乎全是雜訊（US3 情境 6）。全部趨勢一次回傳，切換指標不需再打 API——避免每切一次就讓後端重新載入上萬筆事件。上限 60 點使回應大小有界（18 項 × 60 點，數十 KB）；一週打兩次、每次 5 場的球員，60 點仍涵蓋約一個半月的逐場移動，搭配「全部 vs. 最近」對比已足以回答「有沒有進步」。上限已記入規格 Assumptions。
